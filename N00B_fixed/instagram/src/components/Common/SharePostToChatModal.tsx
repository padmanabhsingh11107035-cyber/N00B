...
export const SharePostToChatModal: React.FC<SharePostToChatModalProps> = ({ currentUser, itemId, itemType, onClose }) => {
  ...
  useEffect(() => {
    ...
    fetchChats().then((chats) => {
      ...
      const dms = chats
        ...
        .filter((x): x is { chatId: string; user: User } => !!x.user);
      setFriends(dms);
      setLoading(false);
    });
    return () => {
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    await Promise.all(
      Array.from(selected).map((chatId: string) =>
        sendMessage(chatId, { sharedPostId: itemId, sharedPostType: itemType }).catch(() => undefined)
      )
    );
    setSending(false);
    setDone(true);
