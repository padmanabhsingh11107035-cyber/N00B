-- NOOB — "share post/reel" message type: send a post or reel into a chat, same shape and same
-- security precedent as the existing "share profile" message type (20260925000040).
--
-- The client only ever sends WHICH post/reel (sharedPostId + sharedPostType); the server looks it up
-- itself and builds the stored snapshot from the real row, respecting the exact same visibility rules
-- (blocks, hidden profiles, private accounts) already used for viewing that post/reel directly — so a
-- link can never be used to leak a caption/thumbnail the recipient wasn't allowed to see anyway.
...
language sql stable set search_path = public as $$
  select jsonb_build_object(
    ...
    'sharedTrack', m.shared_track, 'gameInvite', m.game_invite, 'sharedProfile', m.shared_profile, 'sharedPost', m.shared_post,
    'replyTo', m.reply_to,
...
declare
  ...
  post_row public.posts; reel_row public.reels; first_slide record;
begin
